...
class CmdPreprocessor(Preprocessor):
    ...
    ignores any arguments it does not understand.''')
        argp.add_argument('inputs', metavar = 'input', default = [input], nargs = '*', action = FileAction, help = 'Files to preprocess (use \'-\' for stdin)')
        argp.add_argument('-o', dest = 'output', metavar = 'path', type = argparse.FileType('wt'), default=output, nargs = '?', help = 'Output to a file instead of stdout')
        ...
        try:
            ...
            for i in self.args.inputs:
                i.close()
            if self.args.output != sys.stdout and not isinstance(self.args.output, io.StringIO):
                self.args.output.close()
