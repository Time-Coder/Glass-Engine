...
class Var:
    def __init__(
            self:Self, name:str="", type:str="", location:int=-1,
        ...
        self.layout_kwargs:Dict[str, str] = ({} if layout_kwargs is None else layout_kwargs)
        self.other_qualifiers:List[str] = ([] if other_qualifiers is None else other_qualifiers)
...
class Struct:
    def __init__(self, name:str="", members=None, start_index:int=-1, end_index:int=-1):
        self.name:str = name
...
class FuncCall:
    def __init__(self, name:str="", args=None, call_expression:Union[tree_sitter.Node, None]=None):
    ...
    def signature(self)->str:
        return self.name + "(" + ', '.join([arg.text.decode("utf-8") for arg in self.args]) + ")"
...
class Func:
    def __init__(self, return_type:str="void", name:str="", args=None, start_index:int=-1, end_index:int=-1):
