...
class ShaderParser_:
    ...
    def glsl_parser():
        ...
        dll_file = f"{self_folder}/lib/{machine}/{platform_system}/{bits}/glsl.{dll_suffix}"
    ...
    @staticmethod
    def parse_struct_declaration_list(struct_declaration_list:tree_sitter.Node)->Dict[str, Var]:
            ...
            if type_qualifier.type == "layout_qualifier":
                for layout_qualifier_id in type_qualifier.children:
                    ...
                    if layout_qualifier_id.child_count == 1:
                        ...
                        layout_kwargs[key] = layout_qualifier_id.children[2].text.decode("utf-8")
            else:
    ...
    def parse_single_value_declaration(self, declaration:tree_sitter.Node):
        ...
        is_uniform = False
        if (
            fully_specified_type.child_count == 2 and
            fully_specified_type.children[0].type == "type_qualifier_list" and
            fully_specified_type.children[1].type == "type_specifier"
        ):
            type_qualifier_list = fully_specified_type.children[0]
            type_specifier = fully_specified_type.children[1]
            has_qualifier = True
        elif (
            fully_specified_type.child_count == 1 and
            fully_specified_type.children[0].type == "type_specifier"
        ):
            type_specifier = fully_specified_type.children[0]
            has_qualifier = False

        if has_qualifier:
            layout_args, layout_kwargs, other_qualifiers, in_out = self.parse_qualifiers(type_qualifier_list)
            is_attribute = (
                (layout_kwargs or "attribute" in other_qualifiers) and
                in_out == 'in' and self.shader_type == GL.GL_VERTEX_SHADER
            )
    ...
    def parse_block_declaration(self, declaration):
        ...
        struct = Struct(
            name=type_,
            members=ShaderParser_.parse_struct_declaration_list(struct_declaration_list)
        )
    ...
    def is_single_value_declaration(declaration):
        return (
            declaration.child_count >= 2 and
            declaration.children[0].type == "fully_specified_type" and
            declaration.children[1].type in ["identifier", "array_declarator"]
        )
    
    @staticmethod
    def is_block_declaration(declaration):
        return (
            declaration.child_count >= 5 and
            declaration.children[0].type == "type_qualifier_list" and
            declaration.children[1].type == "identifier" and
            declaration.children[2].type == "{" and
            declaration.children[3].type == "struct_declaration_list" and
            declaration.children[4].type == "}"
        )
    ...
    def is_struct_declaration(declaration):
        return (
            declaration.child_count == 2 and
            declaration.children[0].type == "fully_specified_type" and
            declaration.children[0].child_count == 1 and
            declaration.children[0].children[0].type == "type_specifier" and
            declaration.children[0].children[0].child_count == 5 and
            declaration.children[0].children[0].children[0].type == "struct" and
            declaration.children[0].children[0].children[1].type == "identifier" and
            declaration.children[0].children[0].children[2].type == "{" and
            declaration.children[0].children[0].children[3].type == "struct_declaration_list" and
            declaration.children[0].children[0].children[4].type == "}" and
            declaration.children[1].type == ";"
        )
    ...
    def is_only_qualifiers(declaration):
        return (
            declaration.child_count >= 2 and
            declaration.children[0].type == "type_qualifier_list" and
            declaration.children[0].child_count >= 2 and
            declaration.children[0].children[1].text in [b"in", b"out"] and
            declaration.children[1].type == ";"
        )
    
    def parse_only_qualifiers(self, declaration):
        layout_args, layout_kwargs, other_qualifiers, in_out = self.parse_qualifiers(declaration.children[0])
        ...
        location = (-1 if "location" not in layout_kwargs else eval(layout_kwargs["location"]))
        var = Var(
    ...
    def parse(self, clean_code:str):
        ...
        for child in root_node.children:
            if child.type == "declaration":
                ...
                elif ShaderParser_.is_struct_declaration(declaration): # struct definition
    ...
    def resolve_var(self, current_var:Var, parent:Union[Struct, Func, None]=None, mark_as_used:bool=False):
    ...
    def find_best_match_function(self, func_call:FuncCall, func:Func)->Union[Func, None]:
        ...
        for candidate_func in self.candidate_functions(func_call.name):
            ...
            current_distance = self.type_list_distance(arg_types, candidate_func.arg_types)
    ...
    def analyse_type(self, expression:tree_sitter.Node, func:Func)->str:
        if expression.type == "number_literal":
            ...
            if expression.next_sibling is not None and expression.next_sibling.type == "ERROR":
            ...
            if prefix_type in [
                ...
                if len(suffix_text) == 1:
                    if prefix_type[0] == 'b': return "bool"
                    elif prefix_type[0] == 'i': return "int"
                    elif prefix_type[0] == 'u': return "uint"
                    elif prefix_type[0] == 'v': return "float"
                    elif prefix_type[0] == 'd': return "double"
                else:
            ...
            if (
                func_call_name in GLInfo.atom_type_names or
                func_call_name in self.structs or
                func_call_name in ShaderBuiltins.structs
            ):
    ...
    def type_list_distance(self, type_list1:List[str], type_list2:List[str])->Union[int|None]:
        full_distance = 0
    ...
    def treeshake(self)->str:
        ...
        for struct in self.structs.values():
            if not struct.is_used and struct.start_index != -1 and struct.end_index != -1:
        ...
        for i in range(len(segments_to_be_removed)):
            if i+1 < len(segments_to_be_removed):
                result += self.clean_code[segments_to_be_removed[i][1]:segments_to_be_removed[i+1][0]]
            else:
