...
class ComputeProgram(GPUProgram):
	...
	def _check_work_group_size(work_group_size:tuple)->None:
		...
		if work_group_size[1] > max_work_group_size[1]:
			raise ValueError("y-dimension work group size should not be greater than " + str(max_work_group_size[1]) + ", " + str(work_group_size[1]) + " were given")
		if work_group_size[2] > max_work_group_size[2]:
			raise ValueError("z-dimension work group size should not be greater than " + str(max_work_group_size[2]) + ", " + str(work_group_size[2]) + " were given")
		
		max_work_group_invocations = GLConfig.max_compute_work_group_invocations
		work_group_invocations = work_group_size[0] * work_group_size[1] * work_group_size[2]
	...
	def _check_work_group_count(work_group_count:tuple)->None:
		...
		if work_group_count[1] > max_work_group_count[1]:
			raise ValueError("y-dimension work group size should not be greater than " + str(max_work_group_count[1]) + ", " + str(work_group_count[1]) + " were given")
	...
	def compile(self, file_name:str)->None:
		...
		self._shader_storage_blocks_info = self.compute_shader._shader_storage_blocks_info
