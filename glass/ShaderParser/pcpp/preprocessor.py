...
class Preprocessor(PreprocessorHooks):
    ...
    def add_path(self, path):
        ...
        if relpath is not None:
            self.rewrite_paths += [
                (
                    re.escape(os.path.abspath(path) + os.sep) + "(.*)",
                    os.path.join(relpath, "\\1"),
                )
            ]
        ...
        for argnum, i in macro.str_patch:
            if argnum not in str_expansion:
                ...
                for j in xrange(len(tokens)):
                    if (
                        tokens[j].type in self.t_WS
                        and tokens[j].type != self.t_LINECONT
                    ):
                        tokens[j].value = " "
    ...
    def expand_macros(self, tokens, expanding_from=[]):
        ...
        while i < len(tokens):
            ...
            if t.type == self.t_ID:
                if (
                    t.value in self.macros
                    and t.value not in t.expanded_from
                    and t.value not in expanding_from
                ):
                    ...
                    if m.arglist is None:
                        ...
                        while j < len(tokens) and (
                            tokens[j].type in self.t_WS
                            or tokens[j].type in self.t_COMMENT
                        ):
                            j += 1
                        ...
                        if j == len(tokens) or tokens[j].value != "(":
                            ...
                            if (
                                ...
                                self.on_error(
                                    t.source,
                                    t.lineno,
                                    "Macro %s requires %d arguments but was passed %d"
                                    % (t.value, len(m.arglist), len(args)),
                                )
                                i = j + tokcount
                                ...
                                if (
                                    len(tokens) > j + tokcount
                                    and tokens[j + tokcount].type in self.t_ID
                                ):
    ...
    def evalexpr(self, tokens):
        ...
        try:
            result = self.evaluator(
                tokens, functions=evalfuncts, identifiers=evalvars
            ).value()
            partial_expansion = (
                partial_expansion
                or evalvars.partial_expansion
                or evalfuncts.partial_expansion
            )
            ...
            if not partial_expansion:
                self.on_error(
                    tokens[0].source,
                    tokens[0].lineno,
                    "Could not evaluate expression due to %s (passed to evaluator: '%s')"
                    % (repr(e), "".join([tok.value for tok in tokens])),
                )
            result = 0
    ...
    def parsegen(self, input, source=None, abssource=None):
        ...
        self.include_times.append(
            FileInclusionTime(
                self.macros["__FILE__"] if "__FILE__" in self.macros else None,
                source,
                abssource,
                self.include_depth,
            )
        ...
        for x in lines:
                ...
                try:
                        ...
                        if self.debugout is not None:
                            print(
                                ...
                                % (
                                    ...
                                    dirtokens[0].lineno,
                                    dirtokens[0].value,
                                    "".join([tok.value for tok in args]),
                                ),
                                file=self.debugout,
                            )
                            # print(ifstack)
                    ...
                    if name == "define":
                        ...
                        if enable:
                            ...
                            chunk = []
                            oldfile = (
                                self.macros["__FILE__"]
                                if "__FILE__" in self.macros
                                else None
                            )
                            if (
                                args
                                and args[0].value != "<"
                                and args[0].type != self.t_STRING
                            ):
                        ...
                        if ifstack:
                            if ifstack[
                                -1
                            ].enable:  # We only pay attention if outer "if" allows this
                                if (
                                    enable and not ifpassthru
                            ...
                            if self.debugout is not None:
                                print(
                                    ...
                                    % (
                                        ...
                                        dirtokens[0].source,
                                        dirtokens[0].lineno,
                                        oldifstackentry.startlinetoks[0].source,
                                        oldifstackentry.startlinetoks[0].lineno,
                                        "".join(
                                            [
                                                n.value
                                                for n in oldifstackentry.startlinetoks
                                            ]
                                        ),
                                    ),
                                    file=self.debugout,
                        ...
                        output_unexpanded_line = (
                            self.on_directive_unknown(
                                dirtokens[0], args, ifpassthru, precedingtoks
                            )
                            is None
                        )
            ...
            if (
                not skip_auto_pragma_once_possible_check
                and auto_pragma_once_possible
                and not ifstack
                and not all_whitespace
            ):
                auto_pragma_once_possible = False
                if self.debugout is not None:
                    print(
                        '%d:%d:%d %s:%d Determined that #include "%s" is not entirely wrapped in an include guard macro, disabling auto-applying #pragma once'
                        % (
                            enable,
                            iftrigger,
                            ifpassthru,
        ...
        for i in ifstack:
            self.on_error(
                i.startlinetoks[0].source,
                i.startlinetoks[0].lineno,
                "Unterminated " + "".join([n.value for n in i.startlinetoks]),
            )
        if auto_pragma_once_possible and include_guard and include_guard[1] == 1:
            if self.debugout is not None:
                print(
                    '%d:%d:%d %s:%d Determined that #include "%s" is entirely wrapped in an include guard macro called %s, auto-applying #pragma once'
                    % (
                        enable,
                        iftrigger,
                        ifpassthru,
            ...
            if self.debugout is not None:
                print(
                    "%d:%d:%d %s:%d Did not auto apply #pragma once to this file due to auto_pragma_once_possible=%d, include_guard=%s"
                    % (
                        enable,
                        iftrigger,
                        ifpassthru,
                        self.source,
                        0,
                        auto_pragma_once_possible,
                        repr(include_guard),
                    ),
                    file=self.debugout,
                )
        my_include_time_end = clock()
        self.include_times[my_include_times_idx].elapsed = (
            my_include_time_end - my_include_time_begin
        )
        self.include_depth -= 1
        ...
        if tokens:
            ...
            if tokens[0].value == "<":
                ...
                while i < len(tokens):
                    ...
                    self.on_error(
                        tokens[0].source, tokens[0].lineno, "Malformed #include <...>"
                    )
                    return
                ...
                p = self.on_include_not_found(
                    True,
                    False,
                    self.temp_path[0] if self.temp_path else "",
                    tokens[0].value,
                )
                assert p is None
                return
        ...
        while True:
            ...
            for p in path:
                ...
                if fulliname in self.include_once:
                    if self.debugout is not None:
                        print(
                            'x:x:x x:x #include "%s" skipped as already seen'
                            % (fulliname),
                            file=self.debugout,
                        )
                    if (
                        self.passthru_includes is not None
            ...
            if not mtype:
                ...
                for a in args:
                    ...
                    if len(a) > 1 or a[0].type != self.t_ID:
                        self.on_error(
                            a[0].source, a[0].lineno, "Invalid macro argument"
                        )
                        break
                else:
                    ...
                    m = Macro(
                        name.value,
                        mvalue,
                        [x[0].value for x in args] if args != [[]] else [],
                        variadic,
                    )
    ...
    def parse(self, input, source=None, ignore={}):
        ...
        if isinstance(input, FILE_TYPES):
            ...
            input = input.read()
        self.ignore = ignore
        self.parser = self.parsegen(
            input, source, os.path.abspath(source) if source else None
        )
        ...
        while not done:
            ...
            for n in xrange(len(toks) - 1, -1, -1):
                if toks[n].type in self.t_LINECONT:
                    if (
                        n > 0
                        and n < len(toks) - 2
                        and toks[n - 1].type in self.t_WS
                        and toks[n + 1].type in self.t_WS
                    ):
            ...
            if emitlinedirective and self.line_directive is not None:
                oh.write(
                    ...
                    + " "
                    + str(lastlineno)
                    + ("" if lastsource is None else (' "' + lastsource + '"'))
                    + "\n"
                )
            for tok in toks:
                if tok.type == self.t_COMMENT1:
