...
class sampler2DArray(FBOAttachment):
    ...
    def handle(self):
        ...
        if self._handle == 0:
            ...
            if self._handle == 0:
                raise RuntimeError(
                    f"failed to create sampler2DArray {self._id}'s handle"
                )
    ...
    def bind(self, update_fbo: bool = False, force_update_image: bool = False):
        ...
        if self._wrap_r_changed:
            GL.glTexParameteri(
                GL.GL_TEXTURE_2D_ARRAY, GL.GL_TEXTURE_WRAP_R, self._wrap_r
            )
            self._wrap_r_changed = False
        if self._wrap_s_changed:
            GL.glTexParameteri(
                GL.GL_TEXTURE_2D_ARRAY, GL.GL_TEXTURE_WRAP_S, self._wrap_s
            )
            self._wrap_s_changed = False
        if self._wrap_t_changed:
            GL.glTexParameteri(
                GL.GL_TEXTURE_2D_ARRAY, GL.GL_TEXTURE_WRAP_T, self._wrap_t
            )
        ...
        if self._filter_min_changed:
            GL.glTexParameteri(
                GL.GL_TEXTURE_2D_ARRAY, GL.GL_TEXTURE_MIN_FILTER, self._get_filter_min()
            )
            self._filter_min_changed = False
        if self._filter_mag_changed:
            GL.glTexParameteri(
                GL.GL_TEXTURE_2D_ARRAY, GL.GL_TEXTURE_MAG_FILTER, self._filter_mag
            )
    ...
    def _set_internal_format(self, internal_format):
        ...
        for layer in range(self.layers):
            ...
            if (old_channels, new_channels) in convert_map:
                ...
                self._images[layer] = np.append(
                    self._images[layer],
                    np.zeros(
                        (
                            self._images[layer].shape[0],
                            self._images[layer].shape[1],
                            new_channels - old_channels,
                        ),
                        dtype=self._images[layer].dtype,
                    ),
                    axis=2,
                )
            elif new_channels < old_channels:
            ...
            if old_np_dtype != new_np_dtype:
                if "int" in str(old_np_dtype) and "float" in str(new_np_dtype):
                    self._images[layer] = (self._images[layer] / 255).astype(
                        new_np_dtype
                    )
