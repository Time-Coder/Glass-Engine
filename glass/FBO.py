...
class FBO(BO):
    ...
    @checktype
    def __init__(
        self, width: int = 0, height: int = 0, samples: int = None, layers: int = None
    ):
    ...
    def attach(
        ...
        if GlassConfig.debug:
            ...
            if (
                not (0 <= attach_point < GLConfig.max_color_attachments)
                and not (
                    GL.GL_COLOR_ATTACHMENT0
                    <= attach_point
                    < GL.GL_COLOR_ATTACHMENT0 + GLConfig.max_color_attachments
                )
                and attach_point not in GLInfo.none_color_attachment_types
            ):
        ...
        if isinstance(attachment, type):
            ...
            if attachment_type not in FBO._attachment_types:
                raise TypeError(
                    f"only {FBO._attachment_types} can be attached. {attachment} was given."
                )
            attachment = None
        elif not isinstance(attachment, FBOAttachment):
            raise TypeError(
                f"only instance of FBOAttachment can be attached. {type(attachment)} value was given."
            )
        ...
        if GlassConfig.debug:
            if (
                ...
                and attachment_type
                not in (sampler2DMS, isampler2DMS, usampler2DMS, RBO)
                and not isinstance(
                    attachment, (sampler2DMS, isampler2DMS, usampler2DMS, RBO)
                )
            ):
                raise TypeError(
                    "can only attach (sampler2DMS, isampler2DMS, usampler2DMS, RBO) to multi-samples FBO"
                )

            if (
                self._layers is not None
                and attachment_type not in (sampler2DArray,)
                and not isinstance(attachment, (sampler2DArray,))
            ):
                raise TypeError(
                    f"can only attach (sampler2DArray,) to multi-layers FBO, {attachment_type.__class__.__name__} were given."
                )
        ...
        if attachment is None:
            ...
            if isinstance(
                attachment,
                (sampler2D, isampler2D, usampler2D, samplerCube, sampler2DArray),
            ):
        ...
        if GlassConfig.debug:
            if attach_point == GL.GL_DEPTH_ATTACHMENT:
                if internal_format not in GLInfo.depth_internal_formats:
                    raise ValueError(
                        f"depth attachment internal format should be in {GLInfo.depth_internal_formats}, {internal_format} was given."
                    )
            elif attach_point == GL.GL_STENCIL_ATTACHMENT:
                if internal_format not in GLInfo.stencil_internal_formats:
                    raise ValueError(
                        f"stencil attachment internal format should be in {GLInfo.depth_internal_formats}, {internal_format} was given."
                    )
            elif attach_point == GL.GL_DEPTH_STENCIL_ATTACHMENT:
                if internal_format not in GLInfo.depth_stencil_internal_formats:
                    raise ValueError(
                        f"depth_stencil attachment internal format should be in {GLInfo.depth_stencil_internal_formats}, {internal_format} was given."
                    )
            elif internal_format not in GLInfo.color_internal_formats:
                raise ValueError(
                    f"color attachment internal format should be in {GLInfo.color_internal_formats}, {internal_format} was given."
                )
    ...
    def resize(self, width: int, height: int, samples: int = None, layers: int = None):
        ...
        if (
            ...
            and self._samples == samples
            and self._layers == layers
        ):
            self._can_resize = True
            return False
    ...
    def bind(self):
        ...
        if not self._attachments_attached:
            ...
            for key, attachment in self._color_attachments.items():
                ...
                if isinstance(attachment, (sampler2D, isampler2D, usampler2D)):
                    GL.glFramebufferTexture2D(
                        GL.GL_FRAMEBUFFER,
                        GL.GL_COLOR_ATTACHMENT0 + color_location,
                        GL.GL_TEXTURE_2D,
                        attachment.id,
                        0,
                    )
                    ...
                    raise TypeError(
                        f"not support attachment type {type(attachment).__name__}"
                    )
            ...
            if self._depth_attachment is not None:
                ...
                if isinstance(
                    self._depth_attachment, (sampler2D, isampler2D, usampler2D)
                ):
                    GL.glFramebufferTexture2D(
                        GL.GL_FRAMEBUFFER,
                    ...
                    self._depth_attachment, (sampler2DMS, isampler2DMS, usampler2DMS)
                ):
                    GL.glFramebufferTexture2D(
                        GL.GL_FRAMEBUFFER,
                        GL.GL_DEPTH_ATTACHMENT,
            ...
            if self._stencil_attachment is not None:
                ...
                if isinstance(
                    self._stencil_attachment, (sampler2D, isampler2D, usampler2D)
                ):
                    GL.glFramebufferTexture2D(
                        GL.GL_FRAMEBUFFER,
                        GL.GL_STENCIL_ATTACHMENT,
                        GL.GL_TEXTURE_2D,
                        self._stencil_attachment.id,
                        0,
                    )
                elif isinstance(
                    self._stencil_attachment, (sampler2DMS, isampler2DMS, usampler2DMS)
                ):
                    GL.glFramebufferTexture2D(
                        GL.GL_FRAMEBUFFER,
                        GL.GL_STENCIL_ATTACHMENT,
                    ...
                    self._stencil_attachment, (samplerCube, sampler2DArray)
                ):
                    GL.glFramebufferTexture(
                        GL.GL_FRAMEBUFFER,
                        GL.GL_STENCIL_ATTACHMENT,
            ...
            if self._depth_stencil_attachment is not None:
                ...
                if isinstance(
                    self._depth_stencil_attachment, (sampler2D, isampler2D, usampler2D)
                ):
                    GL.glFramebufferTexture2D(
                        GL.GL_FRAMEBUFFER,
                        GL.GL_DEPTH_STENCIL_ATTACHMENT,
                        GL.GL_TEXTURE_2D,
                        self._depth_stencil_attachment.id,
                        0,
                    )
                elif isinstance(
                    self._depth_stencil_attachment,
                    ...
                    self._depth_stencil_attachment, (samplerCube, sampler2DArray)
                ):
                    GL.glFramebufferTexture(
                        GL.GL_FRAMEBUFFER,
                        GL.GL_DEPTH_STENCIL_ATTACHMENT,
            ...
            if not has_attachments:
                raise RuntimeError(
                    "FBO " + str(self._id) + " do not have any attachments"
                )
            ...
            if color_locations:
                ...
                GL.glDrawBuffers(
                    len(color_attach_points), np.array(color_attach_points)
                )
            else:
    ...
    def copy_from_active(self, *targets):
        ...
        with self:
            ...
            if (
                self._stencil_attachment is not None
                and GL.GL_STENCIL_ATTACHMENT in targets
            ):
                GL.glBlitFramebuffer(
                    viewport[0],
                    viewport[1],
            ...
            if (
                self._depth_stencil_attachment is not None
                and GL.GL_DEPTH_STENCIL_ATTACHMENT in targets
            ):
                GL.glBlitFramebuffer(
                    viewport[0],
                    viewport[1],
    ...
    def draw_to(self, fbo, src_targets, dest_targets=None):
        ...
        with fbo:
            ...
            if (
                self._depth_attachment is not None
                and GL.GL_DEPTH_ATTACHMENT in src_targets
            ):
                GL.glBlitFramebuffer(
                    0,
                    0,
            ...
            if (
                self._stencil_attachment is not None
                and GL.GL_STENCIL_ATTACHMENT in src_targets
            ):
                GL.glBlitFramebuffer(
                    0,
                    0,
            ...
            if (
                self._depth_stencil_attachment is not None
                and GL.GL_DEPTH_STENCIL_ATTACHMENT in src_targets
            ):
                GL.glBlitFramebuffer(
                    0,
                    0,
    ...
    def resolved(self):
        ...
        if self._resolved_fbo is None:
            ...
            for key, attachment in self._color_attachments.items():
                ...
                self._resolved_fbo.attach(
                    key, type_attachment, attachment.internal_format
                )
            ...
            if self._depth_attachment is not None:
                ...
                self._resolved_fbo.attach(
                    GL.GL_DEPTH_ATTACHMENT,
                    type_attachment,
                    self._depth_attachment.internal_format,
                )
            ...
            if self._stencil_attachment is not None:
                ...
                self._resolved_fbo.attach(
                    GL.GL_STENCIL_ATTACHMENT,
                    type_attachment,
                    self._stencil_attachment.internal_format,
                )
            ...
            if self._depth_stencil_attachment is not None:
                ...
                self._resolved_fbo.attach(
                    GL.GL_DEPTH_STENCIL_ATTACHMENT,
                    type_attachment,
                    self._depth_stencil_attachment.internal_format,
                )
        else:
    ...
    def data(self, attach_point: int = 0):
        ...
        if channels == 1:
            ...
            result_data = np.zeros(
                (self._height, self._width, channels), dtype=np_dtype
            )

        GL.glReadPixels(
            0, 0, self._width, self._height, external_format, dtype, result_data
        )
        self._unbind()
