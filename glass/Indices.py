...
class Indices(SameTypeList):
	@checktype
	def __init__(self, _list:(list,np.ndarray)=None, draw_type:GLInfo.draw_types=GL.GL_STATIC_DRAW, dtype=None):
	...
	def _apply_increment(self):
		...
		if patch["old_capacity"] == patch["new_capacity"]:
			for move in patch["move"]:
				self._ebo.memmove(move["old_start"]*self.stride, move["size"]*self.stride, move["new_start"]*self.stride)
		else:
			temp_ebo = EBO()
			temp_ebo.malloc(patch["new_capacity"]*self.stride, self._draw_type)
			self._ebo.copy_to(0, min(patch["old_size"], patch["new_size"])*self.stride, temp_ebo, 0)
			for move in patch["move"]:
		...
		if new_data and patch_update:
			...
			if len_patch_update > 1:
				...
				for update in patch_update:
					...
					temp_ebo.copy_to(src_start*self.stride, size*self.stride, self._ebo, dest_start*self.stride)
			else:
				update = patch_update[0]
				dest_start = update["dest_start"]
				size = update["size"]
				self._ebo.bufferSubData(dest_start*self.stride, size*self.stride, temp_buffer)
	...
	@property
	def buffer(self):
		return bytearray(self.ndarray.tobytes()) + bytearray((self.capacity - len(self))*self.stride)