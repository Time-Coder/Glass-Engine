...
class Box(Mesh):
    ...
    def build(self):
        ...
        # 左面
        position = cgm.vec3(-Lx / 2, Ly / 2, -Lz / 2)
        tex_coord = (
            cgm.vec3(0)
            if self.normalize_st
            else cgm.vec3(0.5 - self.s_per_unit*position.y, 0.5 + self.t_per_unit*position.z, 0)
        )
        vertices[0] = Vertex(
            position=position, normal=cgm.vec3(-1, 0, 0), tex_coord=tex_coord
        )
        ...
        tex_coord = (
            cgm.vec3(0)
            if self.normalize_st
            else cgm.vec3(0.5 + self.s_per_unit*position.x, 0.5 + self.t_per_unit*position.z, 0)
        )
        vertices[8] = Vertex(
            position=position, normal=cgm.vec3(0, -1, 0), tex_coord=tex_coord
        )
        ...
        tex_coord = (
            cgm.vec3(0)
            if self.normalize_st
            else cgm.vec3(0.5 + self.s_per_unit*position.x, 0.5 - self.t_per_unit*position.y, 0)
        )
        vertices[16] = Vertex(
            position=position, normal=cgm.vec3(0, 0, -1), tex_coord=tex_coord
        )
