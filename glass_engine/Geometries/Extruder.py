...
from ..algorithm import (
    line_intersect_plane,
    point_mirror_by_plane,
    polygon_centroid,
    polygon_normal,
    cos_angle_of,
)
...
class Extruder(Mesh):
    ...
    def build(self):
        ...
        if self.__join_style == Extruder.JoinStyle.MiterJoin:
            yield from self.miter_join_build(
                current_section, accumulate_length, section_offset, path
            )
        elif self.__join_style == Extruder.JoinStyle.RoundJoin:
            yield from self.round_join_build(
                current_section, accumulate_length, section_offset, path
            )
        elif self.__join_style == Extruder.JoinStyle.BevelJoin:
            yield from self.bevel_join_build(
                current_section, accumulate_length, section_offset, path
            )
            ...
            if i == 0:
                ...
                if i < len_path - 1:
                    ...
                    for j in range(len_section):
                        point_on_bisection = line_intersect_plane(
                            current_section[j], d1, path[i], plane_normal
                        )
                    ...
                    if should_add_corner:
                        ...
                        for j in range(len_section):
                            point_on_pendicular = line_intersect_plane(
                                current_section[j], d1, path[i], d1
                            )
                            ...
                            if bisection_point_projection < pendicular_point_projection:
                                ...
                                next_point = point_mirror_by_plane(
                    ...
                    for j in range(len_section):
                        current_section[j] = (
                            line_intersect_plane(current_section[j], d1, path[i], d1)
                            + section_offset[j] * d1
                        )
                    ...
                    if should_add_corner:
                        ...
                        for j in range(len_section):
                            current_section[j] = line_intersect_plane(
                                current_section[j], d1, first_hit_point, d1
                            )
