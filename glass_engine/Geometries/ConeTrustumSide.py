...
class ConeTrustumSide(Mesh):

    @checktype
    def __init__(self, bottom_radius:float=2, top_radius:float=1, height:float=1,
    ...
    def build(self):
        ...
        for j in range(n_divide):
            ...
            top = glm.vec3(top_radius * cos_theta,
                           top_radius * sin_theta,
                           height)
            bottom = glm.vec3(bottom_radius * cos_theta,
                              bottom_radius * sin_theta,
                              0)
            ...
            vertex_top_side.tex_coord = glm.vec3(0.5+tex_coord_top_side_radius*cos_theta, 0.5+tex_coord_top_side_radius*sin_theta, 0)

            vertex_bottom_side = Vertex()
            vertex_bottom_side.position = bottom
            vertex_bottom_side.normal = normal
            vertex_bottom_side.tex_coord = glm.vec3(0.5+tex_coord_bottom_side_radius*cos_theta, 0.5+tex_coord_bottom_side_radius*sin_theta, 0)

            vertices[i_vertex] = vertex_top_side # 2*j
            i_vertex += 1

            vertices[i_vertex] = vertex_bottom_side # 2*j + 1
            i_vertex += 1
