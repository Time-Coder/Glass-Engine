...
def init_Dodecahedron(cls):
    ...
    cls.base_positions = [
        cgm.normalize(cgm.vec3(-1, -1, -1)),  # 0
        cgm.normalize(cgm.vec3(1, -1, -1)),  # 1
        cgm.normalize(cgm.vec3(1, 1, -1)),  # 2
        cgm.normalize(cgm.vec3(-1, 1, -1)),  # 3
        cgm.normalize(cgm.vec3(-1, -1, 1)),  # 4
        cgm.normalize(cgm.vec3(1, -1, 1)),  # 5
        cgm.normalize(cgm.vec3(1, 1, 1)),  # 6
        cgm.normalize(cgm.vec3(-1, 1, 1)),  # 7
        cgm.normalize(cgm.vec3(0, -phi2, -phi1)),  # 8
        cgm.normalize(cgm.vec3(0, phi2, -phi1)),  # 9
        cgm.normalize(cgm.vec3(0, phi2, phi1)),  # 10
        cgm.normalize(cgm.vec3(0, -phi2, phi1)),  # 11
        cgm.normalize(cgm.vec3(-phi1, 0, -phi2)),  # 12
        cgm.normalize(cgm.vec3(phi1, 0, -phi2)),  # 13
        cgm.normalize(cgm.vec3(phi1, 0, phi2)),  # 14
        cgm.normalize(cgm.vec3(-phi1, 0, phi2)),  # 15
        cgm.normalize(cgm.vec3(-phi2, -phi1, 0)),  # 16
        cgm.normalize(cgm.vec3(phi2, -phi1, 0)),  # 17
        cgm.normalize(cgm.vec3(phi2, phi1, 0)),  # 18
        cgm.normalize(cgm.vec3(-phi2, phi1, 0)),  # 19
    ]
    ...
    for i, index in enumerate(cls.base_indices):
        ...
        tex_coord2 = None
        if i % 3 == 0:
            tex_coord0 = 0.5 * cgm.vec3(
                1 - math.sin(2 * theta), 1 + math.cos(2 * theta), 0
            )
            tex_coord1 = 0.5 * cgm.vec3(
                1 + math.sin(2 * theta), 1 + math.cos(2 * theta), 0
            )
            tex_coord2 = 0.5 * cgm.vec3(1 + math.sin(theta), 1 + math.cos(theta), 0)
        elif i % 3 == 1:
            tex_coord0 = 0.5 * cgm.vec3(
                1 - math.sin(2 * theta), 1 + math.cos(2 * theta), 0
            )
            tex_coord1 = 0.5 * cgm.vec3(1 + math.sin(theta), 1 + math.cos(theta), 0)
            tex_coord2 = cgm.vec3(0.5, 1, 0)
        elif i % 3 == 2:
            tex_coord0 = 0.5 * cgm.vec3(
                1 - math.sin(2 * theta), 1 + math.cos(2 * theta), 0
            )
            tex_coord1 = cgm.vec3(0.5, 1, 0)
            tex_coord2 = 0.5 * cgm.vec3(1 - math.sin(theta), 1 + math.cos(theta), 0)
    ...
    def build(self):
        ...
        for i, fix_vert in enumerate(Dodecahedron.fixed_vertices):
            ...
            if not self.normalize_st:
                ...
                ) * Dodecahedron.edge_length / sin_pi_5 + cgm.vec3(0.5, 0.5, 0)
