...
class SphericalCap(Mesh):
    ...
    @checktype
    def __init__(self, base_radius:float=1, height:float=0.5,
                 ...
                 n_lat_divide:int=50, name:str="", block=True):
    ...
    def build(self):
        ...
        start_lat = math.asin((base_radius**2-height**2)/(base_radius**2+height**2))
        ...
        for i in range(n_lon_divide):
            ...
            for j in range(n_lat_divide):
                ...
                vertex_top = Vertex()
                vertex_top.tangent = 2*math.pi*radius*cos_phi*glm.vec3(-sin_theta, cos_theta, 0)
                vertex_top.bitangent = math.pi*radius*glm.vec3(-sin_phi*cos_theta, -sin_phi*sin_theta, cos_phi)
                vertex_top.normal = glm.vec3(cos_phi*cos_theta, cos_phi*sin_theta, sin_phi)
            ...
            vertex_bottom.position = glm.vec3(base_radius*cos_theta, base_radius*sin_theta, 0)
