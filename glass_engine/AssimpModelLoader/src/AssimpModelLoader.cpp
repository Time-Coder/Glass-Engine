...
Model AssimpModelLoader::load(const std::string& file_path, unsigned flags)
{
    Model model;
    ...
    std::vector<Texture>& textures)
{
    ...
    {
        aiString file_path;
        assimp_material->GetTexture(texture_type, j, &file_path);
        if (file_path.length > 1 && file_path.data[0] == '*')
        {
            int texture_index = std::stoi(std::string(file_path.data + 1));
