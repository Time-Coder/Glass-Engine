...
class SkyDome(Mesh):
    ...
    def build(self):
        ...
        for i in range(n_lon_divide):
            ...
            for j in range(n_lat_divide):
                ...
                vertex.position = glm.vec3(cos_phi*cos_theta, cos_phi*sin_theta, sin_phi)
    ...
    def program(self):
        if self.__program is None:
            ...
            self.__program.compile(self_folder + "/glsl/Pipelines/skydome/skydome.vs")
            self.__program.compile(self_folder + "/glsl/Pipelines/skydome/skydome.fs")
