...
def cube_info(cube_file_name: str):
    ...
    N = 0
    regx = re.compile(
        r"^\s*(?P<number1>[-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?)\s+(?P<number2>[-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?)\s+(?P<number3>[-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?)\s*$"
    )
    for line in lines:
        line = line.strip()
...
def apply_cube(
    ...
    if isinstance(src_image, str):
        ...
        src_image = cv2.flip(
            cv2.cvtColor(
                cv2.imread(src_image, cv2.IMREAD_UNCHANGED), cv2.COLOR_BGR2RGB
            ),
            0,
        )
    ...
    for j in range(2):
        for k in range(2):
            values[0.5, j, k] = (1 - R_rear) * values[0, j, k] + R_rear * values[
                1, j, k
            ]
    ...
    values[0.5, 0.5, 0.5] = (1 - B_rear) * values[0.5, 0.5, 0] + B_rear * values[
        0.5, 0.5, 1
    ]
    ...
    if dest_file_name is not None:
        out_image = cv2.flip(
            cv2.cvtColor(
                (np.clip(result, 0, 1) * 255).astype(np.uint8), cv2.COLOR_RGB2BGR
            ),
            0,
        )
        cv2.imwrite(dest_file_name, out_image)
