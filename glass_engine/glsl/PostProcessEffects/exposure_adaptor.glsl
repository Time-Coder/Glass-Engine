...
vec4 post_process(sampler2D screen_image, vec2 tex_coord)
{
    ...
    if (!camera.lens.auto_exposure)
    {
        if (frag_coord.x == 0 && frag_coord.y == 0)
        {
            current_luma = pow((1.0/camera.lens.exposure) - 0.5, 2.0);
            memoryBarrier();
        }

        color.rgb = camera.lens.exposure * color.rgb;
    }
    else if (!camera.lens.local_exposure)
    {
        ...
        {
            if (_current_luma != 0 && camera.lens.exposure_adapt_time > 1E-6)
            {
                float sgn = sign(target_luma - _current_luma);
                float a = log(10)/camera.lens.exposure_adapt_time;
