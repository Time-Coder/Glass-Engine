...
#include "SpotLight_lighting_nobindless.glsl"
...
vec3 get_specular_color(InternalMaterial internal_material, Camera CSM_camera, vec3 out_dir, vec3 frag_pos, vec3 frag_normal) {
    ...
    for(int i = 0; i < n_dir_lights; i++)
        specular_color += get_specular_color(dir_lights[i], internal_material, CSM_camera, out_dir, frag_pos, frag_normal);
    for(int i = 0; i < n_point_lights; i++)
        specular_color += get_specular_color(point_lights[i], internal_material, out_dir, frag_pos, frag_normal);
    for(int i = 0; i < n_spot_lights; i++)
        specular_color += get_specular_color(spot_lights[i], internal_material, out_dir, frag_pos, frag_normal);
...
vec3 lighting(InternalMaterial internal_material, Camera CSM_camera, vec3 camera_pos, vec3 frag_pos, vec3 frag_normal) {
    ...
    for(int i = 0; i < n_dir_lights; i++)
        out_color3 += lighting(dir_lights[i], internal_material, CSM_camera, camera_pos, frag_pos, frag_normal);
    for(int i = 0; i < n_point_lights; i++)
        out_color3 += lighting(point_lights[i], internal_material, camera_pos, frag_pos, frag_normal);
    for(int i = 0; i < n_spot_lights; i++)
