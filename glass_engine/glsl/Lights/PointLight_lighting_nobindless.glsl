...
bool get_lighting_info(in PointLight light, inout InternalMaterial material, in vec3 to_camera, in vec3 frag_pos, in vec3 frag_normal, out vec3 to_light, out float attenuation) {
...
vec3 lighting(PointLight light, InternalMaterial material, vec3 camera_pos, vec3 frag_pos, vec3 frag_normal) {
    vec3 to_camera = normalize(camera_pos - frag_pos);
    vec3 to_light; float attenuation;
    if (!get_lighting_info(light, material, to_camera, frag_pos, frag_normal, to_light, attenuation))
        return vec3(0);
    vec3 lighting_color = lighting(to_light, to_camera, frag_normal, material);
    return attenuation * light.color * lighting_color;
}

vec3 get_diffuse_color(PointLight light, InternalMaterial material, vec3 view_dir, vec3 frag_pos, vec3 frag_normal) {
...
vec3 get_specular_color(PointLight light, InternalMaterial material, vec3 out_dir, vec3 frag_pos, vec3 frag_normal) {
