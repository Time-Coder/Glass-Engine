...
bool get_lighting_info(in SpotLight light, inout InternalMaterial material, in vec3 to_camera, in vec3 frag_pos, in vec3 frag_normal, out vec3 to_light, out float cutoff, out float attenuation) {
...
vec3 lighting(SpotLight light, InternalMaterial material, vec3 camera_pos, vec3 frag_pos, vec3 frag_normal) {
    vec3 to_camera = normalize(camera_pos - frag_pos);
    vec3 to_light; float cutoff, attenuation;
    if (!get_lighting_info(light, material, to_camera, frag_pos, frag_normal, to_light, cutoff, attenuation))
        return vec3(0);
...
vec3 get_diffuse_color(SpotLight light, InternalMaterial material, vec3 view_dir, vec3 frag_pos, vec3 frag_normal) {
...
vec3 get_specular_color(SpotLight light, InternalMaterial material, vec3 out_dir, vec3 frag_pos, vec3 frag_normal) {
