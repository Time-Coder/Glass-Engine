...
void main()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        vec3 view_tangent = world_dir_to_view(camera, gs_in[i].world_TBN[0]);
        vec3 view_bitangent = world_dir_to_view(camera, gs_in[i].world_TBN[1]);
        vec3 view_normal = world_dir_to_view(camera, gs_in[i].world_TBN[2]);
