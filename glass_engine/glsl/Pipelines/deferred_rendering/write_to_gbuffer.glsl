    ...
    out vec4 env_center_and_mixed_value, out uvec4 mixed_uint)
{
    ...
    if (internal_material.shading_model == SHADING_MODEL_COOK_TORRANCE || internal_material.shading_model == SHADING_MODEL_PBR)
    ...
    if (internal_material.shading_model == SHADING_MODEL_FLAT || internal_material.shading_model == SHADING_MODEL_GOURAUD)
