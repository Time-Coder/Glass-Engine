...
vec4 texture2DArrayFXAA(sampler2DArray screen_image, vec2 tex_coord, int layer)
{
    vec2 rcpFrame = 1.0 / textureSize(screen_image, 0).xy;
    vec4 uv = vec4(tex_coord, tex_coord - (rcpFrame * (0.5 + FXAA_SUBPIX_SHIFT)));

    vec4 rgbNW = texture2DArray(screen_image, vec3(uv.zw, layer));
    vec4 rgbNE = texture2DArray(screen_image, vec3(uv.zw + vec2(1,0)*rcpFrame.xy, layer));
    vec4 rgbSW = texture2DArray(screen_image, vec3(uv.zw + vec2(0,1)*rcpFrame.xy, layer));
    vec4 rgbSE = texture2DArray(screen_image, vec3(uv.zw + vec2(1,1)*rcpFrame.xy, layer));
    vec4 rgbM  = texture2DArray(screen_image, vec3(uv.xy, layer));
    ...
    vec4 rgbA = (1.0/2.0) * (
        texture2DArray(screen_image, vec3(uv.xy + dir * (1.0/3.0 - 0.5), layer)) +
        texture2DArray(screen_image, vec3(uv.xy + dir * (2.0/3.0 - 0.5), layer))
    );

    vec4 rgbB = rgbA * (1.0/2.0) + (1.0/4.0) * (
        texture2DArray(screen_image, vec3(uv.xy + dir * (0.0/3.0 - 0.5), layer)) +
        texture2DArray(screen_image, vec3(uv.xy + dir * (3.0/3.0 - 0.5), layer))
    );
