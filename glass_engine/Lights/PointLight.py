...
class PointLight(Light):
    ...
    def __update_coverage(self):
        ...
        inv_e = 1 / epsilon
        self._coverage = (
            -self._K1 + math.sqrt(self._K1**2 + 4 * self._K2 * (inv_e - 1))
        ) / (2 * self._K2)
    ...
    def coverage(self, coverage: float):
        def func(t):
            ...
            epsilon = 0.01
            inv_e = 1 / epsilon
            return (-K1 + math.sqrt(K1**2 + 4 * K2 * (inv_e - 1))) / (
                2 * K2
            ) - 10 * coverage
        ...
        self._K1 = 3.651720188286232 / math.pow(
            t - 1.379181323137789, 0.956790970458513
        )
        self._K2 = 27.101525310782399 / math.pow(
            t - 2.191989674193149, 1.727016118197271
        )
        for flat in self._flats:
            flat.K1 = self._K1
            flat.K2 = self._K2
...
class FlatPointLight(FlatLight):
    ...
    def __init__(self, point_light: PointLight):
        self.abs_position = cgm.vec3(0, 0, 0)
...
class PointLights(BlockHostClass):
