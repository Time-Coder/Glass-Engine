...
class CommonRenderer(Renderer):
    ...
    def update_spot_lights_depth(self):
        ...
        if (
            not self._meshes_cast_shadows
            and not self._lines_cast_shadows
            and not self._points_cast_shadows
        ):
            return

        for spot_light in self.scene.spot_lights:
            if not spot_light.generate_shadows or not spot_light.need_update_depth_map:
                continue
                ...
                with spot_light.depth_fbo:
                    ...
                    if self._lines_cast_shadows:
                        ...
                        for mesh, instances in self._lines_cast_shadows:
                            self.spot_light_depth_lines_program["material"] = (
                                mesh.material
                            )
                            self.spot_light_depth_lines_program["back_material"] = (
                                mesh._back_material
                            )
                    ...
                    if self._points_cast_shadows:
                        ...
                        for mesh, instances in self._points_cast_shadows:
                            self.spot_light_depth_points_program["material"] = (
                                mesh.material
                            )
                            self.spot_light_depth_points_program["back_material"] = (
                                mesh._back_material
                            )
    ...
    def update_point_lights_depth(self):
        ...
        for point_light in self.scene.point_lights:
            if (
                not point_light.generate_shadows
                or not point_light.need_update_depth_map
            ):
                continue
            ...
            with GLConfig.LocalConfig(
                ...
                with point_light.depth_fbo:
                    ...
                    if self._lines_cast_shadows:
                        self.point_light_depth_lines_program["point_light"] = (
                            point_light
                        )
                        for mesh, instances in self._lines_cast_shadows:
                            self.point_light_depth_lines_program["material"] = (
                                mesh.material
                            )
                            self.point_light_depth_lines_program["back_material"] = (
                                mesh._back_material
                            )
                            mesh.draw(self.point_light_depth_lines_program, instances)

                    if self._points_cast_shadows:
                        self.point_light_depth_points_program["point_light"] = (
                            point_light
                        )
                        for mesh, instances in self._points_cast_shadows:
                            self.point_light_depth_points_program["material"] = (
                                mesh.material
                            )
                            self.point_light_depth_points_program["back_material"] = (
                                mesh._back_material
                            )
                ...
                with dir_light.depth_fbo:
                    ...
                    if self._lines_cast_shadows:
                        ...
                        for mesh, instances in self._lines_cast_shadows:
                            ...
                            self.dir_light_depth_lines_program["back_material"] = (
                                mesh._back_material
                            )
                    ...
                    if self._points_cast_shadows:
                        ...
                        for mesh, instances in self._points_cast_shadows:
                            ...
                            self.dir_light_depth_points_program["back_material"] = (
                                mesh._back_material
                            )
    ...
    def gen_env_map(self, mesh, instances):
        ...
        max_bake_times = max(
            mesh.material.env_max_bake_times, mesh._back_material.env_max_bake_times
        )
        ...
        for instance in instances:
                    ...
                    with env_map_fbo:
                        ...
                        if self._transparent_points:
                            ...
                            for other_mesh, other_instances in self._transparent_points:
                                self.gen_env_map_draw_points(
                                    other_mesh, other_instances
                                )
            ...
            with GLConfig.LocalConfig(
                ...
                with env_OIT_blend_fbo:
                    ...
                    self.env_OIT_blend_program.draw_triangles(start_index=0, total=6)
    ...
    def draw_transparent(self):
        if (
            not self._transparent_meshes
            and not self._transparent_points
            and not self._transparent_lines
        ):
            return
